...
func NewTracerProvider(ai *conf.AppInfo, conf *conf.Bootstrap) (trace.TracerProvider, func(), error) {
	...
	return tp, func() {
		...
		if err2 != nil {
			fmt.Printf("close trace provider(%s) error(%s))", conf.Trace.Endpoint, err2)
		}
	}, nil
}
...
func InsertVersionRow(next schema.Applier) schema.Applier {
	return schema.ApplyFunc(func(ctx context.Context, conn dialect.ExecQuerier, plan *migrate.Plan) error {
		for _, c := range plan.Changes {
			m, ok := c.Source.(*atlas.AddTable)
			if ok && m.T.Name == version.Table {
				// Insert version row.
...
func NewEntClient(conf *conf.Bootstrap, m *Metric) (*ent.Client, func(), error) {
	...
	if err = ec.Schema.Create(
		...
		schema.WithApplyHook(InsertVersionRow),
	); err != nil {
