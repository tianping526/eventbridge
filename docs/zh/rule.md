...
- Pattern 对 Event 特定字段进行匹配，同一个字段的多个匹配规则之间是"或"的关系，
...
- Pattern 对 Event 特定字段进行匹配，同一个字段的多个匹配规则之间，也可以是"与"的关系，
